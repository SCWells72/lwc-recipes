...
describe('c-editRecordScreenAction', () => {
    ...
    it('Test populates name from getRecord wire', async () => {
        ...
        const firstNameEl = element.shadowRoot.querySelector<LightningInput>(
            "[data-field='FirstName']"
        );
        const lastNameEl = element.shadowRoot.querySelector<LightningInput>(
            "[data-field='LastName']"
        );
        // Emit data from @wire
    ...
    it('Test update record from updateRecord wire on save', async () => {
        // Create component
        const element = createElement<EditRecordScreenAction>('c-edit-record-screen-action', {
            is: EditRecordScreenAction
        });
        document.body.appendChild(element);

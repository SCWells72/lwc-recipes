...
describe('c-lds-notify-record-update-available', () => {
    ...
    it('populates name from getRecord wire', async () => {
        ...
        const firstNameEl = element.shadowRoot.querySelector<LightningInput>('.first-name');
        const lastNameEl = element.shadowRoot.querySelector<LightningInput>('.last-name');
        // Emit data from @wire
    ...
    it('should update contact and call notifyRecordUpdateAvailable', async () => {
        ...
        const inputEl = element.shadowRoot.querySelector<LightningButton>('lightning-button');
        inputEl.click();
        ...
        expect((<jest.MockInstance<any, any>><unknown>updateContact).mock.calls[0]).toEqual([
    ...
    it('displays an error toast on update record error', async () => {
        ...
        (<jest.MockInstance<any, any>><unknown>updateContact).mockRejectedValue(UPDATE_CONTACT_ERROR);

...
describe('c-api-spread', () => {
    ...
    function setInputElementValues(element: Element, firstName: string, lastName: string) {
        // lightning-input doesn't mirror its properties as attributes so
        // can't use an attribute query selector.
        (element.shadowRoot
            .querySelectorAll<LightningInput>('lightning-input'))
            .forEach((input) => {
    ...
    it('renders c-child component with default values', () => {
        ...
        const childEl = element.shadowRoot.querySelector<Child>('c-child');

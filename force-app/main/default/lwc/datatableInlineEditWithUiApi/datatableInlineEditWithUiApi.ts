...
import { RecordInputRepresentation, updateRecord } from 'lightning/uiRecordApi';
...
export default class DatatableInlineEditWithUiApi extends LightningElement {
    ...
    async handleSave(event: CustomEvent<LightningDatatable>) {
        // Convert datatable draft values into record objects
        const records = event.detail.draftValues.slice().map((draftValue: any) => {
        ...
        try {
            ...
            const recordUpdatePromises = records.map((record: RecordInputRepresentation) =>
                updateRecord(record)
            );
